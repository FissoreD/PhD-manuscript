...
when applied to any list of seven elements, regardless of the instantiation of
\mlI{'a}. The implementation is identical for all types.

Parametric polymorphism is widely used and highly practical in functional
programming. However, it is often insufficient when modeling mathematical
structures. In mathematics, the symbol $+$ has a consistent intuitive meaning,
and no ambiguity arises in expressions such as $1 + 2$ or $\pig + e$. In the
former, $+$ denotes addition over integers; in the latter, it denotes addition
over real (or rational) numbers. The same symbol is used for operations that
share common algebraic properties.
...
The possibility of sharing symbol is often called symbol overloading.
