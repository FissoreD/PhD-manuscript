...
A more sophisticated encoding of \rocq terms and rules
that, on a first approximation, would reshape the compiled rule for \cI{Pimpl} in
as follows:
...
In this chapter we identify a minimal language \Fo{} in which the problems
...
The $\lambda$Prolog code discussed in this chapter can be accessed at the
address \githubUrl.
...
%minimal, for example, we omit the \eI{all} quantifier of CIC we used
%in the example in Section~\ref{sec:intro} together with the type notation of
%terms carried by the \eI{lam} constructor.
%
...
\noindent
Note that the symbol \eI{|} separates the head of a list from the tail;
that \eI{forall2} applies a binary predicate to two lists (of equal
length); and that \eI{=>} is $\lambda$Prolog syntax for
...
\noindent
In the code above, the syntax \eI{pi x y\..} is syntactic sugar for
iterated \eI{pi} abstraction, as in \eI{pi x\ pi y\..}.
The auxiliary predicate \eI{fold6} folds a predicate
