\chapter{\texorpdfstring{$\eta\beta$}{eta beta}-unification support for \Fo and \Ho}
\label{chap:ho-for-free}
\label{sec:intro}

We aim to implement a
form of proof search known as type-class resolution~\cite{wadler1989-tc,sozeau2008-tc}
...
Elpi meta programming language~\cite{tassi2018},
a dialect of $\lambda$Prolog, already used to extend
Coq in various ways~\cite{tassi2018,tassi2019,tassi2023,fissore2023}.
...
We take as an example the \cI{Decision} and \cI{Finite} type classes
from the Stdpp
library~\cite{iris-junk}.
...
Note that rule 3 features a second-order parameter \cI{P} that represents
...
\llambda~\cite{miller1991-pf}.


\noindent
Following the standard syntax of $\lambda$Prolog~\cite{miller2012},
the meta-level binding of a variable \eI{x} in an expression
\eI{e} is written as <<\eI{x\ e}>>, while square brackets delimit a
list of terms separated by comma. For example, the term
<<\cI{~$\forall$~y:t, nfact y 3}>> is encoded as follows:
...
parameters; \eI{:-} separates the rule's head from the premises, and
<<\eI{pi w\ p}>> introduces a fresh nominal constant \eI{w}
for the premise \eI{p}.
...
that, on a first approximation, would reshape rule r3 in \ref{fig:rules-elpi} as follows:
...
\noindent
Since \eI{Pm} is a higher-order unification variable
of type \eI{tm -> tm},
with \eI{x}
in its scope, the unification problem
admits one solution:
...
Constraint Handling Rules (CHR) to operate on them~\cite{tassi2019-1,chr15}.
...
they are part of a new goal (see \cite[section 4.3]{tassi2019-1}).
