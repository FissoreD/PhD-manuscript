...
\forall n,$ if all terms in \foUnifPb belongs to \llambda
$$
...
meta-language, while \mlam encodes object-level abstraction of in the HOAS
...
Note that \eI{move} relies crucially on invariant \cref{inv:uvaarity},
...
A link \linketaM{\Gamma}{X}{t} is removed from \linkStore when
  $X$ becomes  in a substitution $\sigma$. Let $y\in\Gamma$. There are two cases:
\begin{enumerate}
...
\begin{proof}[Proof sketch]
  ...
  $s$ in the object language that is not in \notllambda. This category
