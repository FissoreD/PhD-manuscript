\chapter{\texorpdfstring{$\eta\beta$}{eta beta}-unification support for meta-programs}
...
in \elpi, but the $\eta\beta$-conversion rules are part of the \elpi unification
...
$\eta\beta$-unification problem is due to the instance \cI{Pimpl}.
...
% Taken from \stdpp, another example dealing with $\eta\beta$-unification can be
...
In the body of the rule, we have added a new higher-order \elpi variable \eI{Qm} which
has the type \eI{term ~$\to$~ term}. In the recursive call to \eI{tc-Provable}
we pass as argument the variable \eI{Qm} applied to \eI{p}, allowing the the
unification problem \eI{Qm p = p} to have a solution, namely, \eI{Qm = x\ x}.

The role of the \eI{link} predicate is to relate the \elpi variable \eI{Qm} back
to the variable \eI{Q}. For instance, when \eI{Qm} is assigned, then
\eI{Q} is assigned to the HOAS term \cI{fun x => x}. With this encoding, the
...
% back to the domain \eI{tm} of Coq terms, obtaining the expected solution: 
% % \ref{solution:intro}:
...
The general idea of this compilation is to identify at compilation time all
the subterm that could create unification problems up to $\beta\eta$. These
subterms $t$ are replaced by unification variables $v$ to ease unification in the
meta-langauge and an additional premise is added to relate $t$ to $v$.

In our paper~\cite{fissore2024}, we present a general framework using a
dedicated syntax for both the HOAS encoding of the terms of the object langauge
and the one of the meta language. In this chapter, we prefer to stick to the
HOAS of the \rocq langauge and use the \elpi syntax for the meta language.

Since one of the main issue concerning this chapter is related to the scope of
\elpi variable, we will explicitely put as subscript the their scopes. For
example, an elpi variable noted $\eI{X}_{xy}$ means that the $x$ and $y$ are in
the scope of \eI{F}. We distingish pure \elpi variables from variables mapped to
\rocq existential variables by prefixing \rocq variables with the \cI{?} symbol.
By hypothesis every \rocq meta-variable in the following sections have empty
scope, i.e. they are existentially quantified before any other free variable.

% The idea of our compilation strategy is presented via two languages called
% called \rocq for object language and \elpi for the meta language. 
% We detail the encoding of \rocq in \elpi, and we show that
...
% the object language that features $\beta\eta$-conversion.

Section~\ref{sec:problem-statement} states the problem formally and gives
the\marginpar{TODO} intuition behind our solution; \cref{sec:simulation} sets up
a basic simulation of first-order logic programs, \cref{sec:llam} and
\cref{sec:eta} extend it to higher-order logic programs in the pattern fragment
while \cref{sec:beta} goes beyond the pattern fragment. Section
\ref{sec:implementation} discusses the implementation in \elpi. 
...
% a language \rocq with a rich equational
% theory and a \elpi language with a simpler one.

% Our encoding of \rocq and \elpi is given in the \elpi language. In
% \cref{code:common-terms}, we provide in the syntax of the terms of \rocq, called
% \eI{to} and the terms of \elpi, called \eI{tm}. As claimed before, our framework
% is general, and therefore not proper to \rocq and \elpi. In our case, \eI{to}
% corresponds to the \eI{term} type of \rocq terms in \elpi and \eI{tm}
% corresponds to the constructs of the $\lambda$-calculus of \elpi.

% The only difference between the two term representations is the encoding of
% variables. Variables contain a pointer to a memory address. In \rocq,
% the variables are first-order, i.e. they have no term in their scope, while in
% \elpi, the variables have a scope which is represented as a list of term. We use
% this encoding, since we implement and test the full unification strategy in an
...
%   \end{subfigure}
%   \caption{The \rocq and \elpi languages}\vspace{0.3em}
%   \label{code:common-terms}
% \end{figure}

% \textbf{Notational conventions} 
%
% When we write \elpi terms outside code blocks we follow the
...
% Here are few examples:\\\marginpar{da rivedere: usare altra sintassi}
...
% When it is clear from the context, we shall use the same syntax for \rocq terms
% (although we never use subscripts for unification variables).
% We use $s$, $s_1$, \ldots for terms in \rocq and $t$, $t_1$, \ldots for
% terms in \elpi.

\section{Equational theories and unification}

% \paragraphTwo{Term equality: \Eo and \Ee}
% For both languages, we define the equational theory
...
% but different to any other term. The implementation is given in~\cref{fig:feq}.
...
%   \caption{\rocq equality implementation}
%   \label{fig:feq}
% \end{figure}%
% %
% \begin{elpicode}
...
% \noindent
We define two equality procedure, which test term equality over ground terms,
extended with reflexivity for unification variables, i.e. a unification variable
is equal to itself but different to any other term. Equality of terms of HOAS
terms is noted \Eo and simulates the equality of these terms modulo
$\eta\beta$-conversion in the \rocq langauge, whereas equality of HOAS terms in
the \elpi language is noted \Ee.

For example, \eI{fun _ x\ app[?F, x]}~\Eo~\eI{?F}, and \eI{app [fun _ x\ x,
~<<3>>~]}~\Eo~\eI{~<<3>>~}, but \eI{fun _ x\ app[?F, x]}~\nEe~\eI{?F}, and
\eI{app [fun _ x\ x, ~<<3>>~]}~\nEe~\eI{~<<3>>~}.

The main point in showing these equality tests is to remark how
weaker \Ee is, and to identify the the special
treatment needed for the $\eta\beta$-conversion rules.

% Another impo
% \paragraphTwo{Substitution: $\rho s$ and $\sigma t$}
...
% We shall use $\rho$ for \rocq substitutions,
% and $\sigma$ for the \elpi ones.
% For brevity, in this section, we consider
% the substitution for \rocq and \elpi identical.
...
We finally need to define the unification symbols of the \rocq and \elpi
language. \Ue is the unification of \elpi. Its signature is
%
% \input{code/ue_type}
...
The symbol \Uo stands fo the unification of \rocq. Its signature is:
%
\begin{elpicode}
  ~\PYG{k+kd}{type} \PYG{n+nf}{(\Uo)} \PYG{k+kt}{tm -> tm -> subst -> subst -> o}~.
\end{elpicode}
...
% Both \Ue and \Uo satisfy \cref{prop:good-unif}.

\noindent
Both \Uo in \rocq and \Ue in \elpi are good unifications for terms in \llambda.
...
We represent a logic program \emph{run} in \rocq as
...
each \rocq-term $s \in \foUnifPb$ is translated to an \elpi-term $t \in \hoUnifPb$.
...
The variable map connects unification variables in \elpi to variables
in \rocq and is used to ``decompile'' the substitution, that is written
$\D{\sigma}{m}{l}{\rho}$. Links are an accessory piece of information whose
description is deferred to \cref{sec:nutshell}.

Then we simulate each run in \rocq with a run in \elpi as follows:
%
...
for the two \elpi terms being unified at step $p$, and we write $\hoUnifPb_p$
...
\end{proposition}
\noindent
In particular, this property guarantees that a \emph{failure} in the \rocq run
is matched by a failure in \elpi \emph{at the same step}. We consider this
...
to debug a logic program in \rocq by looking at its execution trace in
\elpi.
...
Given a set of terms $X$, % \subseteq \elpi$,
$$
...
Since each \elpi unification variable occurs together with a scope,
...
\noindent
A solution to a \rocq variable is, instead, a plain term, that is, \eI{fsubst}
is an abbreviation for \eI{mem to}.
%We call \eI{fsubst} the memory of \rocq, while we call \eI{subst}
%the one of \elpi.
%
...
\begin{invariant}[Unification-variable arity]\label{inv:uvaarity}
  Each variable \eI{A}
  in \elpi has a (unique) arity \eI{N} and each occurrence
...
%the memory.
%  To ease the comparison we split \rocq dereferencing into a
...
% operation in \elpi, namely \eI{deref}. On the contrary \eI{napp}
% has no corresponding operation in \elpi, and only ensures that
...
applies the substitution to a \elpi term. Notice how assignments are moved
...
need a link since \elpi variables faithfully represent
...
% \section[Compilation: fo\_tm to tm]{Basic simulation of \rocq in \elpi}
...
the other.
In order to bring back the substitution from \elpi to \rocq the compiler
builds a ``memory map'' connecting the \rocq variables to the \elpi ones.
...
A step in \elpi consists in unifying two terms and reconsidering all
...
% \rocq to \elpi is not lossy, it can be reversed. The statement
...
What we have to show is that whenever two different \rocq
terms can be made equal by a substitution $\rho$,
we can produce this $\rho$ by finding
a $\sigma$ via \Ue{} on the corresponding \elpi terms and by decompiling it.
If we look at the syntax of \rocq, the only interesting case is
...
of the compiler. \foUnifPb represents the input unification problems in \rocq
...
For example, $\hoUnifPb_2$ corresponds to the \elpi problem $t_3 \Ue t_4$.
...
we %need to 
compile the \rocq term $X\appsep x$ into the \elpi term $A_x$.
%,which is \eI{uva A [x]}.
...
\end{lemma}
  ...
  \begin{proof}[Proof sketch]
  If we look at the \rocq terms, there is one more interesting case,
  namely \eI{o-app[o-uva X|W] ~\Uo~s} when \eI{W}
  are distinct names compiled to $\vec{w}$. In this case
  the \elpi problem is $Y_{\vec{w}} \Ue t$ that succeeds with
...
$\lambda x. t \appsep x$. The equational theory of \rocq identifies these terms,
...
% terms and, consequently replace them with fresh \elpi unification variables at
...
\begin{proof}[Proof sketch] %\todo{to reformulate}
  ...
  \rocq unification problem of the form $s_1 \Uo s_2$
...
\begin{proposition}[\mapStore is a bijection]
  After compilation, for each \rocq-variable $X$ in \foUnifPb{} and for each
  \elpi-variable $A$ in \hoUnifPb{} there is exactly one entry $X \mapsto A^n$
  in \mapStore.
\end{proposition}

\noindent
Note that running \Ue{} may require allocating new variables in \elpi as
...
\begin{theorem}[Fidelity with \mapdeduplication]
  ...
  if $\ \foUnifPb$ contains the same \rocq-variable used at different arities,
...
\begin{proof}[Proof sketch]
  Let $X$ be a \rocq-variable appearing in two different subterms $s_1$ and $s_2$.
  ...
  After \mapdeduplication we have two distinct \elpi-variables for $X$, say $A$ and $C$,
  ...
  we work with \wellb terms, this unification succeeds iff it does in \rocq.
...
hence the resulting \rocq substitution is
...
%   \rocq term has the shape \eI{o-app[o-uva M | Ag]} where \eI{Ag} is a
...
\begin{theorem}[Fidelity in \rocq]
...
\begin{proof}[Proof sketch]
  ...
  for the $i-1$ time,
  each variable $X$ corresponding to a \maybeeta subterm in the original \rocq 
  ...
  corresponding unification in \rocq would (it is called outside its domain).
\end{proof}
...
are solved. The final \elpi substitution is $\sigma = \{~A _{} \mapsto \lambda x.x ~;~
...
plays the role of \elpi as well.
...
Constraint Handling Rules (CHR) to operate on them~\cite{chr-elpi,chr15}.
...
We considered mechanizing our results with Abella~\cite{abella},
...
higher order
combinators like \eI{map} or \eI{forall2} is surely
possible, but 
