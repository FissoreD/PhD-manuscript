\chapter{\texorpdfstring{$\beta\eta$}{eta beta}-unification support for meta-programs}
...
We take from our paper~\cite{fissore2024} the sentence which better 
...
% \noindent
% Unfortunately this encoding of rule \emph{r3} in \ref{fig:rules-elpi} does not work
...
% \emph{r3} in \ref{fig:rules-elpi} on the encoding of the goal below given here
...
% \end{elpicode}
% \noindent
% In turn, this redex prevents rule \emph{r2} in \ref{fig:rules-elpi} from backchaining properly since
...
\noindent
Note that \eI{move} strongly relies on invariant \ref{inv:uvaarity}: the length
...
the \eI{inctx} container. % (see~\ref{data:inctx} also used for \eI{subst}).
...
Sections \ref{sec:eta} and \ref{sec:beta} add rules to \eI{progress1}
...
We add the following rule before rule $r_@$ in~\ref{fig:comp-fo}, where
...
\end{lemma}
  ...
  the \Ho problem is $Y_{\vec{w}} \Ue t$ that succeeds with
   ...
   Thanks to rule $r_{\beta_l}$ in \ref{fig:feq} we have
...
rule that can play a role is $r_{\eta_l}$ in \ref{fig:feq}, and note that
...
The following rule is inserted just before rule $r_\lambda$ in~\ref{fig:comp-fo}.
...
\begin{proof}[Proof sketch] %\todo{to reformulate}
  ...
  \begin{itemize}
    ...
    \item $s_2 \notin \maybeeta$,
          ...
          If $s_1$ and $s_2$ are equal thanks to $r_{\eta_l}$ in \ref{fig:feq} then
          progression $p$ unifies $\lambda x.t_1$ with the $\eta$-redex of $t_2$ namly
          $\lambda x.t_2\appsep x$, otherwise they are equal thanks to rule $r_{\lambda\lambda}$ in \ref{fig:feq}
          ...
          since rule $r_{\lambda\lambda}$ in \ref{fig:feq} or rule $r_{\eta_l}$ in \ref{fig:feq} move under it, eventually finding two
...
lacks rules $r_{\beta_l}$ and $r_{\beta_r}$ in \ref{fig:feq}.
...
We insert this rule
just before rule $r_@$ in \ref{fig:comp-fo}
...
% Remark that \fullref{inv:uvaarity} makes it possible to relocate \eI{T2}
