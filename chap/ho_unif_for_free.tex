...
shown in \cref{code:common-terms}. Terms of \Fo{} are called as \eI{otm}, while
the HOAS encoding of these terms in \Ho{} are written as \eI{mtm}. 
...
terms. By contrast, the constructor \eI{mapp} encodes the application of
...
$\beta$-equivalence.
This is because, in the object language, the term \eI{oapp[olam x\ x, con"3"]} 
and \eI{con"3"} are definitionally equal.
%
% \input{code/feq}
\begin{figure}
...
meta-language, while \eI{mlam} encodes object-level abstraction of in the HOAS
of \Ho, in the same way \eI{muva} represents application of
the meta-language, and \eI{mapp} encodes object-level applications in the HOAS
of \Ho.

In contrast, assignments for variables in \Fo are represented as plain
terms. In particular, \eI{fsubst} is an abbreviation for
\eI{memory mtm}. In this setting, the unification
...
\begin{center}
\eI{abs x\ val (uva A [x] ~$=_{\llambda}$~ app[uva F [x], con "a"])}
\end{center}
...
auxiliary function \eI{comp-lam}. This procedure recursively calls
...
Since the \eI{olam} node
carries no additional information (other than the function body),
each \eI{abs} node can be decompiled to a \eI{olam} one.

However, if the \eI{olam} node was carrying more information,
