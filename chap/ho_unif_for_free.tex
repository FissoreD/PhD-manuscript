...
\cI{Provable (impl (atom a) (atom a))} can be solved by the \rocq type-class solver
using the instance \cI{Pimpl}. On the contrary, \elpi fails: the rule above
would be used on the query \eI{tc-Provable {{impl (atom a) (atom a)}} S} but the
sub-query \eI{tc-Provable {{atom a}} {{lp:Q lp:p}}} will fails since not
unifying with the local rule \eI{tc-Provable {{atom a}} p}.
...
problems in the pattern fragment (\cref{def:pattern-fragment}). Although 
...
query \eI{tc-Provable {{imp (atom a) (atom a)}} S} has a solution.
...
% \begin{elpicode}
% P = \mlam A x\ mapp [mcon"nfact", x, mcon"3"]
% \end{elpicode}
...
% \begin{elpicode}
% decision (mapp [                   P                  , w])
% decision (mapp [\mlam A (a\ mapp [mcon"nfact", a, mcon"3"]), w])
% \end{elpicode}
% \noindent
...
% \begin{elpicode}
% mapp [ \mlam A (a\ mapp [mcon"nfact", a, mcon"3"]) , x] =
% mapp [ mcon"nfact"                             , N, NF]
% \end{elpicode}
...
is encoded as
\eI{~\mapp~ [Q, p]}.\marginpar{TODO: syntax coloring \ref{code:common-terms}}
...
\begin{proof}
  By the definition of \eI{comp}, the compilation precedure is a simple mapper
...
% For brevity we omit the code\marginpar{TODO}
...
\begin{proof}[Proof sketch]
  ...
  need to conseder the case of terms \maybeeta. By \cref{lem:maybeeta},
...
$X$ becomes  in a substituion $\sigma$. Let $y\in\Gamma$. There are two cases:
\begin{enumerate}
