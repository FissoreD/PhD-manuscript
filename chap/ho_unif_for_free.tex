...
richer, automatic proof search procedures typically rely, for efficiency and
predictability, on a unification procedure restricted to $\beta\eta$-equivalence
and the higher-order pattern fragment. Outside this fragment, \rocq's
unification algorithm employs heuristics. For example, the unification problem
\eI{P (Q 3) = f (g 3)} succeeds with the substitution $\sigma = \{\eI{P} \mapsto
\eI{f}, \eI{Q} \mapsto \eI{g}\}$. However, this problem admits many incomparable
solutions, such as $\sigma = \{\eI{P} \mapsto \lambda x.\eI{f (g 3)}, \eI{Q}
\mapsto \lambda x.\eI{100}\}$, illustrating the non-deterministic search space
explored by higher-order unification algorithms such as Huet's~\cite{huet75}.
...
\eI{Q} is assigned to the HOAS term \eI{fun `dummy_name` T (x\ x)} where \eI{T}
is the type of the bound variable \eI{x}. With this encoding, the
...
In particular, \cref{sec:problem-statement} states the problem formally and gives the
...
\Cref{sec:implementation} discusses the implementation in \elpi.
...
\begin{figure}
  \begin{subfigure}{0.47\textwidth}
    ...
    \begin{elpicode}
      kind ~\otm~ type.
      type ~\oapp~ list ~\otm~ ~$\to$~ ~\otm~.
      ~\PYG{k+kd}{type}~ ~\olam~ (~\otm~ ~$\to$~ ~\otm~) ~$\to$~ ~\otm~.
      type ~\ocon~ string ~$\to$~ ~\otm~.
      type ~\ouva~ addr ~$\to$~ ~\otm~.
    \end{elpicode}
  \end{subfigure}
  \begin{subfigure}{0.47\textwidth}
    ...
    \begin{elpicode}
      kind ~\mtm~  type.
      type ~\mapp~ list ~\mtm~ ~$\to$~ ~\mtm~.
      ~\PYG{k+kd}{type}~ ~\mlam~ (~\mtm~ ~$\to$~ ~\mtm~) ~$\to$~ ~\mtm~.
      type ~\mcon~ string ~$\to$~ ~\mtm~.
      type ~\muva~ addr ~$\to$~ list ~\mtm~ ~$\to$~ ~\mtm~.
    \end{elpicode}
  \end{subfigure}
...
$X$ is a set of terms.
We write $\sigma \subseteq \sigma'$ for substitution extension as in~\cref{def:sub-extension}.
...
\newcommand{\unifcompleteX}[4]{\ensuremath{
  % \forall #3_1 #3_2, %
    ...
    #4 \to
      % \forall \rho, %
        \rho #3_1 #1 \rho #3_2  %
          \to \exists \rho', #3_1 #2 #3_2 \arr \rho' \land \mathrm{mgu (\rho #3_1, \rho #3_2) = \rho'}
}}
...
$\mathcal{D}$ satisfies the following properties:%\noteD{use $\sigma$, not $\rho$}
  \begin{align}
    \mathrm{Completeness}:\ & \unifcorrectX{\EqualRel}{\UnifRel}{t}{\{~t_1,~ t_2~\} \subseteq \mathcal{D}}\\
    \mathrm{Correctness}:\ & \unifcompleteX{\EqualRel}{\UnifRel}{t}{\{~ t_1,~ t_2 ~\} \subseteq \mathcal{D}}
  \end{align}
when $\mathrm{mgu}$ ensure that $\rho'$ is the most general unifier between $t_1$ and $_2$
under the initial substitution $\rho$.
\end{proposition}  

\noindent
The meta and (resp. object) language of choice are expected to provide 
an implementation of \Ue (resp. \Uo) that is a good unification for \Ee (resp. \Eo)
in \llambda.
...
The predicate \eI{m-alloc} allocates a fresh \Ho variable while preserving
the arity information carried by its argument \eI{Ag}. In particular,
\eI{Ag} should be understood as an input-output parameter: the predicate does
not choose an arbitrary arity, but creates a variable whose arity is fixed by
the current compilation phase, see for example \cref{fig:comp-fo}.
...
\linketa; this detail is discussed in \cref{sec:invariant1}.
...
\noindent
As mentioned in~\cref{sec:nutshell}, the compiler
...
\Cref{sec:eta,sec:beta} add rules to \eI{progress1}
...
where we store the arity (that is a very simple function type).
...
\noindent
The proposition above states that the translation from \Fo{} to \Ho{} is not
lossy: it can be reversed. The statement is made somewhat convoluted by the
fact that we hide the procedure for decompiling a term and instead state the
property in terms of decompiling substitutions.
...
there exists a substitution $\rho$ such that $\rho (\lambda x.r) \Eo r'$ and
$r'$ is not a $\lambda$-abstraction.
...
\maybeeta, it is compiled into \eI{~\mlam~ G} and replaced by a fresh
variable \eI{A}. This variable sees all names that are free in
\eI{~\mlam~ G}, and it is linked to \eI{~\mlam~ G} through a
\linketa. \Cref{inv:linklhs} is satisfied for this link. 
Moreover:
...
\begin{proof}[Proof sketch]
  ...
  the rule in \cref{fig:com-maybeeta} is used. The output is a variable and
  its scope is a list of distinct variables, 
  therefore, the new terms is \wellb.
\end{proof}
...
\begin{enumerate}
  \item if $\sigma X = \lambda x.s$ we call ``run $(X \Ue{} t)\ \sigma$'';
  \item if $\sigma X = a$ or $\sigma X = y$ or $\sigma X = f \appsep a_1\ldots a_n$
    we unify the $\eta$-redex of $X$ with $t$, that is we call ``run
    $(\lambda x.X \appsep x\Ue{} t)\ \sigma$''.
\end{enumerate}
\end{definition}

The idea behind \progressetaleft is that the link is triggered whenever the
suspended variable of the \maybeeta link is assigned a term. In this
situation, we need to simulate either the $r_{\eta_l}$ rule or the
$r_{\lambda\lambda}$ rule from \cref{fig:feq}, depending on whether the head
constructor of the term $\sigma X$ is a $\lambda$-abstraction.

In the first case, the head of the term $\sigma X$ is a $\lambda$-abstraction.
Therefore, we simulate the $r_{\lambda\lambda}$ rule by calling the unification
procedure on the two terms. In the second case, the term $t$ must be an
$\eta$-redex. To simulate the unification performed in the object language by
$r_{\eta_l}$, we $\eta$-expand the term $\sigma X$ and then unify the two terms
using \Ue.
...
\begin{theorem}[Fidelity in $\wellb_{\beta \eta}$] \label{lemma:fidelity-maybeeta}
  ...
  % if the memory map is bijective then
  the introduction 
...
\begin{proof}[Proof sketch] %\todo{to reformulate}
  ...
  \begin{itemize}
    ...
    \item $s_2 \notin \maybeeta$. In this case, $s_2$ is compiled to $t_2$, and the
    unification step $u$ assigns $t_2$ to $X$, thereby triggering
    \progressetaleft. If $s_1 \Uo s_2$ succeeds, then either $r_{\eta_l}$ or
    $r_{\lambda\lambda}$ from \cref{fig:feq} is applied. In the former case,
    the progression $p$ unifies $\lambda x.t_1$ with the $\eta$-redex of
    $t_2$, namely $\lambda x.t_2\appsep x$. The link progression therefore
    attempts to unify $t_1$ with $t_2\appsep x$. Since both terms belong to
    $\maybebeta$, they unify in \Ho by \cref{th:fidelity-beta} and the
    hypothesis that $s_1 \Uo s_2$. In the latter case, $t_2$ has the form
    $\lambda x.t_2'$, where $t_2' \in \maybebeta$. Hence, the link
    progression unifies $t_1$ with $t_2'$, and again the unification succeeds
    by \cref{th:fidelity-beta}.

    Conversely, if $s_1 \Uo s_2$ fails, then neither $r_{\eta_l}$ nor
    $r_{\lambda\lambda}$ succeeds. Depending on the shape of $t_2$,
    \progressetaleft attempts to unify $t_1$ either with $t_2$ or with its
    $\eta$-redex. In both cases, the unification is performed between terms
    in $\maybebeta$. Since $s_1 \Uo s_2$ fails by hypothesis, the
    corresponding higher-order unification also fails by
    \cref{th:fidelity-beta}.
  \end{itemize}
...
After compilation, \mapStore defines a bijective correspondence between
\Fo{}-variables in \foUnifPb{} and \Ho{}-variables in \hoUnifPb{}. In
particular, for every entry $X \mapsto A^n \in \mapStore$, there is no
distinct variable $X' \neq X$ such that $X' \mapsto A^n \in \mapStore$, and
no distinct variable $A'^n \neq A$ such that $X \mapsto A'^n \in \mapStore$.
\end{proposition}
...
The resolution of $\hoUnifPb_1$ assigns $f$ to $A$. This wakes up
...
Other systems apply heuristics and commit to that solution. This is the case for
the unification algorithm \rocq.
...
$X a$ to $Y$.  
This heuristic is is used by Rocq's type-class resolution and vanilla tactics
(like ``apply''), through Rocq's ``first-order'' unification heuristic.
