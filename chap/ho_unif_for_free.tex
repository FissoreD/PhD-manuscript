...
\begin{proof}[Proof sketch] %\todo{to reformulate}
  ...
  \begin{itemize}
    ...
    \item $s_2 \notin \maybeeta$. In this case, $s_2$ is compiled to $t_2$, and
    the unification step $u$ assigns $t_2$ to $X$, thereby triggering
    \progressetaleft. If $s_1 \Uo s_2$ succeeds, then either $r_{\eta_l}$ or
    $r_{\lambda\lambda}$ from \cref{fig:feq} is applied. In the former case, the
    progression $p$ unifies $\lambda x.t_1$ with the $\eta$-redex of $t_2$,
    namely $\lambda x.t_2\appsep x$. The link progression therefore attempts to
    unify $t_1$ with $t_2\appsep x$. Since both terms belong to $\wellb_{\beta
    \eta}$, they unify in \Ho by \cref{th:fidelity-beta}. In the latter case,
    $t_2$ has the form $\lambda x.t_2'$, where $t_2' \in \wellb_{\beta \eta}$.
    Hence, the link progression unifies $t_1$ with $t_2'$, and again the
    unification succeeds by \cref{th:fidelity-beta}.
    ...
    in $\wellb_{\beta \eta}$. Since $s_1 \Uo s_2$ fails by hypothesis, the
